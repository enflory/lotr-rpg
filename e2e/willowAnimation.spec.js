...
// Observe the actual sprites, including hidden ones, once per rendered frame.
// A direct position jump instead of a walk/tumble makes these checks fail.
async function observe(page) {
  await page.evaluate(() => {
    const s = window.__game.scene.getScene('WorldScene');
    window.__willowMotion = { maxStep: 0, moved: {}, samples: 0 };
    const sprites = [s.player, ...s.followers];
    let previous = sprites.map((p) => ({ x: p.x, y: p.y }));
    const sample = () => {
      if (!window.__willowMotion) return;
      const next = sprites.map((p) => ({ x: p.x, y: p.y }));
      next.forEach((p, i) => {
        const d = Math.hypot(p.x - previous[i].x, p.y - previous[i].y);
        window.__willowMotion.maxStep = Math.max(window.__willowMotion.maxStep, d);
        const k = sprites[i].getData('key') || 'frodo';
        window.__willowMotion.moved[k] = (window.__willowMotion.moved[k] || 0) + d;
      });
...
test('capture moves the actual hobbits and waits for the player to help Sam', async ({ page }) => {
  ...
  expect(motion.maxStep).toBeLessThan(16);
