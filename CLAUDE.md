...
The Barrow-downs are rendered from a height field rather than from tiles. `journeyMap.js` provides `smoothNoise` (two-octave value noise, used to raise the hill mask) and `wind` (a Bresenham carve that steps both axes, so a route reads as a worn path instead of a staircase); `barrowdowns.js` runs a `settle` pass that erodes one-tile spurs and fills pinholes before anything is placed. `downsScenery.js` then blurs the solid-tile mask, samples it per pixel with two octaves of grain, and bakes a canvas texture: hill bodies shaded by height with a lit northern skyline, a dark near face, contour banding and a cast shadow on the turf below. The chalk track and the East Road are painted the same way from their own smoothed masks, which is why the base `DOWN_GRASS`/`DOWN_SLOPE`/`DOWN_HEATHER` tiles are deliberately flat: they are only a base coat, and any tile the relief does not own (a road, a milestone) must be listed in `paintable` or it will be erased. `GREAT_STONE` is its own tile so the waymark the road climbs to is unmistakable among the scattered uprights; a pair of `STANDING_STONE` two tiles apart on one row is drawn as the gate.

`barrowEvent.js` owns the downs, the barrow and the morning after, with the same beat contract as the Willow. The mist is raised inside the sleep beat (nothing else runs while a dialogue is open) and held up by `downsFog` afterwards, so a reload mid-scene finds clear noon and replays it. Progress is three staged doors, none of which can be walked past: the great stone (`downs_stone`), the gate stones (`downs_gate`, which separates the party by walking them into the mist rather than hiding them on a coordinate test), and the hollow beyond (`downs_voices`). `heldKeys` is the single source of truth for who is out of sight, so Continue rebuilds the same picture. Checkpoints follow the story — `stone`, then `gate` — not the doorway the party came in by. In the mist, straying more than three tiles off the chalk for four seconds brings up a drifting wisp leaning toward the next landmark, and a banner after twelve. The groping hand is a container of a forearm plus five separately animated digits that routes round the wall spur and crawls at the player; `barrow_wake` and the rising on `barrowhill` are automatic beats, and a beat with no dialogue behind it releases control itself. The summoning verse Tom teaches in `house_farewell` is the same four lines Frodo sings in `barrow_call`.
