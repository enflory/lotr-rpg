...
- **`src/events/gandalfEvent.js`**: Gandalf walks off down the Hill once his farewell has been heard (`gandalfLeft`), and the doorstep is empty from then on (`gandalfGone`). The Ring reveal, the errand for Sam and the farewell are one conversation, set together — split across two, a player could hear the first and walk away without ever seeing him leave. He shows no banner of his own: the objective banner from the conversation is still up, and a wizard walking away says the rest.
- **`src/events/waggonEvent.js`**: the night ride to the Ferry. `route()` is built on demand — the Marish zone imports this module, so reading its lane constants at module load would read them before the zone has defined them. It runs **east along the causeway and nowhere else**: the lane turns a right angle at the farm and a cart drawn from the side cannot turn one, so climbing aboard is covered by a short fade and the whole of the drive is straight. Legs are eased at both ends, the cart rumbles and its wheels roll, and `setFacing(-1)` flips it for the road home or it reverses the whole way. It draws up short of the landing so the party steps down _east_ of it and it can leave without driving through them. Merry walks up out of the fog carrying the lantern rather than appearing at the landing afterwards. The ride ends by setting `rodeWaggon`, which is where `ferryEvent.js` picks up.
