...
Chapter 1 vertical slice is playable end to end: Bag End (interior + exterior), Hobbiton with the Party Field to the west, the Green Dragon interior, and the Woody End. Story: Bilbo's farewell party prologue in the Party Field (fireworks, the speech, the flash-vanish, a seventeen-year time skip — `src/events/partyEvent.js`) → Gandalf reveals the Ring → fetch Sam (joins as follower) → East Road → hide from the Black Rider in the ferns → Gildor → Bucklebury Ferry sign ("to be continued"). Presetting the `prologueDone` flag before ENTER on the title skips the prologue (used by e2e). Chapter 1 still wants: Green Hill Country as a middle zone, mushroom/pickup items, Farmer Maggot and the Marish, a save system (flags → localStorage), more Hobbiton NPCs, and scripted movement (Gandalf leaving).
